...
module.exports = ({ topic = 'jsreport', accessKeyId, secretAccessKey, subscription = subscriptionName, region = 'eu-west-1', logger }) => {
  if (!accessKeyId) {
  ...
  return {
    async init () {
      ...
      logger.debug(`Ensuring sns topic ${topic} exists`)
      const topicRes = await sns.createTopicAsync({ Name: topic })
      topicArn = topicRes.TopicArn

      logger.debug(`Ensuring sqs queue ${subscription} exists`)
